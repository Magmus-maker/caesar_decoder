def decrypt_cesar_cipher(text, shift):
    #Дешифрование текста методом Цезаря для английского языка.
    decrypted_text = ''
    for char in text:
        if char.isalpha():
...
def load_dictionary(file_name):
    #Загрузка словаря из файла для английского языка.
    dictionary = set()
...
def main():
    # Загрузка словаря
    eng_dictionary = load_dictionary('en_US.txt')
    ...
    words_required = int(words_total * 0.8)

    # Перебор всех возможных сдвигов и дешифрование текста
    shift = 0
    while shift < 26:
        decrypted_text = decrypt_cesar_cipher(encrypted_text, shift)
        count_matching_words = count_words_in_dictionary(decrypted_text, eng_dictionary)
